...
その一方で誤り位置関数の計算は一位に決まるのでこのような曖昧や実装の不備などを考慮する必要がない。
やはりグレブナ基底ガ必要になるのだろうか？
結局職人のカンで理論に背いた値を入力したところ正解率が上がった。特定部位に連続して現れる数に本当の答えがあることがわかった。
これを数式化すれば、多数決論理の決定率を向上させることができる。つまり、特定ブロックに信頼度を設定することだ。
これは既存の理論に欠陥がある可能性を意味する。データ全体の頻度より、特定ブロックに正しい答えが出現する可能性が高いのだ。
そのブロックを数式て特定できれば、多数決論理にさらなる進展が見られるだろう。
